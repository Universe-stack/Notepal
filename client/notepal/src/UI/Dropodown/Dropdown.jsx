...
const DropdownMenuDemo = (props) => {
  ...
  const handleDelete= async (e)=>{
    await axios.delete(`http://localhost:8800/notes/${divId}`)
    .then(response => {
      console.log('Post deleted successfully');
      updateNotesAfterDeletion(divId)
      navigate('/allNotes');
    })
    .catch(error => {
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content className="DropdownMenuContent" sideOffset={5}>
          <DropdownMenu.Item className="DropdownMenuItem" onClick={handleDelete}>
            Delete Note <div className="RightSlot">⌘+T</div>
          </DropdownMenu.Item>
          <DropdownMenu.Item className="DropdownMenuItem">
            Create New <div className="RightSlot">⌘+N</div>
          </DropdownMenu.Item>
          <Link to={`/allNotes/${divId}`} style={{ textDecoration: 'none', color: 'white'}}>
          <DropdownMenu.Item className="DropdownMenuItem">
            Edit <div className="RightSlot">⇧+⌘+N</div>
          </DropdownMenu.Item>
          </Link>
          <DropdownMenu.Sub>
